...
const GIFToBase64GUI = ({ onBack }) => {
  ...
  const handleFileSelect = async (droppedFiles = null) => {
    ...
    if (droppedFiles) {
      ...
      files = allFiles.filter((f) => f.toLowerCase().endsWith('.gif'));
    ...
    <div className="tool-container-full">
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        <div className="tool-breadcrumbs">
          ...
          <span style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <a onClick={onBack}>{t("GIF 转换器")}</a>
          </span>
          <span style={{ display: 'flex', alignItems: 'center' }}>
            <span style={{ margin: '0 8px', color: 'var(--text-secondary)' }}>/</span>
            <span className="current">GIF To BASE64</span>
          </span>
        </div>
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("GIF To BASE64 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>{t("\u2014\u2014 一款专业工具，可将 GIF 动图转换为 Base64 编码字符串，方便在代码中直接引用。")}</div>
        </div>
      </div>
      ...
      <div className="tool-content-scroll">
        <div className="tool-main-layout video-converter-layout">
          <div className="tool-left-panel">
            <FileUploaderV2
              ...
              showAudioInfo={false}
              hidePreview={true}
              uploadPlaceholder={t("将您的 GIF 文件拖拽到此处")} />
          ...
          <div className="tool-right-panel">
            <SettingsPanel title={t("转换说明")}>
              <div className="settings-info-box">
                <h4>{t("关于 GIF 转 BASE64")}</h4>
                <p>{t("Base64 编码可以将图片数据直接嵌入到 HTML 或 CSS 中，减少 HTTP 请求，提高页面加载速度。")}</p>
                <p>{t("生成的")}<code>.base64</code>{t("文件包含完整的 Data URL（例如：")}<code>data:image/gif;base64,...</code>{t("），您可以直接将其复制到代码中使用。")}</p>
                <p style={{ marginTop: '10px', fontSize: '12px', color: '#666' }}>{t("提示：Base64 编码会使文件体积增大约 33%，建议仅对较小的 GIF 或需要减少请求数的场景使用。")}
        ...
        <ConfirmationModal
          ...
          title={t("取消转换")}
          message={t("确定要取消当前的转换任务吗？已生成的文件将被删除。")}
