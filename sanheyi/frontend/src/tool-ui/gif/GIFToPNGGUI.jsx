...
const GIFToPNGGUI = ({ onBack }) => {
  ...
  const handleRemoveFile = (index) => {
    ...
    if (activeFile && selectedFiles[index] === activeFile) {
      setActiveFile(null);
    }
  };
  ...
  const confirmCancel = async () => {
    ...
    if (targetPaths.length === 0 && currentFileRef.current) {
      ...
      if (outputDir) {
        ...
        // GIFToPNGConverter outputs both a temp directory and a final ZIP
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("GIF To PNG 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px' }}>{t("一款专业工具，可将 GIF 动图转换为 PNG 图像序列，并支持自定义参数。")}</div>
        </div>
      </div>
