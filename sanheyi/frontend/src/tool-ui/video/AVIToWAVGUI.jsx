...
const AVIToWAVGUI = ({ onBack }) => {
  ...
  const handleFileSelect = async (droppedFiles = null) => {
    ...
    if (droppedFiles) {
      ...
      files = allFiles.filter((f) => f.toLowerCase().endsWith('.avi'));
      if (files.length < allFiles.length) {
        showAlert('提示', '已自动过滤非 AVI 文件');
      }
  ...
  return (
    <div className="tool-container-full">
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("AVI To WAV 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px' }}>{t("一款专业工具，可从AVI视频中提取WAV音频，并支持自定义编码参数。")}</div>
        </div>
      </div>
        ...
        <div className="tool-main-layout video-converter-layout">
          <div className="tool-left-panel">
            <FileUploaderV2
              ...
              uploadPlaceholder={t("将您的AVI文件拖拽到此处")} />
      ...
      <ConfirmationModal
        ...
        title={t("取消转换")}
        message={t("确定要取消当前的转换任务吗？已转换的部分将被删除。")}
