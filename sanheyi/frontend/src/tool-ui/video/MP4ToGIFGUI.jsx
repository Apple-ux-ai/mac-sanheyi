...
const MP4ToGIFGUI = ({ onBack }) => {
  ...
  const handleFileSelect = async (droppedFiles = null) => {
    ...
    const mp4Files = filesToProcess.filter((f) => {
    ...
    const uniqueFiles = mp4Files.filter((f) => {
      const path = typeof f === 'string' ? f : f.path;
      return !selectedFiles.some((s) => (typeof s === 'string' ? s : s.path) === path);
    });
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("MP4 To GIF 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>{t("\u2014\u2014 将 MP4 视频转换为高质量 GIF 动图，支持自定义质量、帧率和时间范围。")}</div>
        </div>
      </div>
