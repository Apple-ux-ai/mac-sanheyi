...
const MP4ToMP3GUI = ({ onBack }) => {
  ...
  const handleFileSelect = async (droppedFiles = null) => {
    ...
    const mp4Files = filesToProcess.filter((file) => {
      const path = typeof file === 'string' ? file : file.path;
      return path.toLowerCase().endsWith('.mp4');
    });

    if (mp4Files.length < filesToProcess.length) {
      showAlert('提示', '已自动过滤非 MP4 文件');
    }

    if (mp4Files.length === 0 && filesToProcess.length > 0) {
      showAlert('提示', '请选择有效的 MP4 文件');
      return;
    }

    // Remove duplicates
    const uniqueNewFiles = mp4Files.filter((file) => {
  ...
  return (
    <div style={{ width: '100%', position: 'relative', height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("MP4 To MP3 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>{t("\u2014\u2014 专业级 MP4 转 MP3 工具，支持批量提取音频和高质量输出。")}</div>
        </div>
      </div>
        ...
        <div className="tool-main-content video-converter-layout">
          <div className="tool-left-panel">
            <FileUploaderV2
              ...
              showAudioInfo={true}
              uploadPlaceholder={t("将您的 MP4 文件拖拽到此处")}
              results={results}
          ...
          <div className="tool-right-panel">
            <SettingsPanel title={t("转换选项")}>
              ...
              <div className="setting-info-box" style={{ marginTop: '12px', padding: '10px', backgroundColor: '#f0f9ff', borderRadius: '6px', fontSize: '12px', color: '#0369a1', border: '1px solid #bae6fd' }}>{t("提示：较高的比特率通常意味着更好的音质，但文件体积也会相应增大。")}
      ...
      <ConfirmationModal
        ...
        title={t("取消转换")}
        message={t("确定要取消当前的转换任务吗？已生成的文件将保留。")}
