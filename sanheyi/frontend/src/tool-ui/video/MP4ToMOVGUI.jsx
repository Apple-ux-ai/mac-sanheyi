...
import { api } from '../../services/api';import { t } from '@/i18n';
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("MP4 To MOV 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>{t("\u2014\u2014 专业级 MP4 转 MOV 工具，支持批量转换和高质量输出。")}</div>
        </div>
      </div>
          ...
          <div className="tool-right-panel">
            <SettingsPanel title={t("转换选项")}>
              <SettingPresets
                ...
                onSelect={(p) => {
                  setPreset(p);
                  if (p === '低质量') {setQuality(40);setAudioBitrate('96k');setResolution('640x360');} else
                  if (p === '中等质量') {setQuality(70);setAudioBitrate('128k');setResolution('1280x720');} else
                  if (p === '高质量') {setQuality(90);setAudioBitrate('320k');setResolution('1920x1080');} else
                  if (p === '社交媒体') {setQuality(80);setAudioBitrate('192k');setResolution('1080x1920');}
                }}
                columns={2} />
