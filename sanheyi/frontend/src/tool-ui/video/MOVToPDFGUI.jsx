...
const MOVToPDFGUI = ({ onBack }) => {
  ...
  const handleFileSelect = async (droppedFiles = null) => {
    ...
    if (droppedFiles) {
      ...
      files = await api.openFileDialog([{ name: 'MOV 视频', extensions: ['mov'] }]);
    } else {
  ...
  const handleWebFileChange = (e) => {
    ...
    if (!activeFile) setActiveFile(names[0]);
  };
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
            ...
            <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("MOV To PDF 转换器")}

            </h1>
          </div>
          <div
            className="header-desc"
            style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>{t("\u2014\u2014 将 MOV 视频帧提取并转换为 PDF 文档，支持自定义页面方向与批量处理。")}
          ...
          <div className="tool-right-panel">
            <SettingsPanel title={t("转换选项")}>
              ...
              <SettingSlider
                ...
                valueDisplay={t("提取原视频的 ${frameInterval}% 帧", { "frameInterval": frameInterval })} />
