...
const MP4ToAVIGUI = ({ onBack }) => {
  ...
  const handleFileSelect = async (droppedFiles = null) => {
    ...
    if (!activeFile && uniqueNewFiles.length > 0) {
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("MP4 To AVI 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>{t("\u2014\u2014 专业级 MP4 转 AVI 工具，支持批量转换和高质量输出。")}</div>
        </div>
      </div>
