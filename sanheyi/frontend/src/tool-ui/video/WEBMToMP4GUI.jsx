...
const WEBMToMP4GUI = ({ onBack }) => {
  ...
  const handleFileSelect = async (droppedFiles = null) => {
    let files = [];
    if (droppedFiles) {
      const allFiles = Array.from(droppedFiles).map((f) => f.path || f.name);
      files = allFiles.filter((f) => f.toLowerCase().endsWith('.webm'));
      if (files.length < allFiles.length) {
        showAlert('提示', '已自动过滤非 WEBM 文件');
      }
    } else if (api.isAvailable()) {
      const selectedPaths = await api.openFileDialog([
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        <div className="tool-breadcrumbs">
          ...
          <span style={{ display: 'flex', alignItems: 'center' }}>
            ...
            <a onClick={onBack}>{t("WEBM 转换器")}</a>
          </span>
          <span style={{ display: 'flex', alignItems: 'center' }}>
            <span style={{ margin: '0 8px', color: 'var(--text-secondary)' }}>/</span>
            <span className="current">WEBM To MP4</span>
          </span>
        </div>
        <div className="header-text" style={{ marginTop: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("WEBM To MP4 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px' }}>{t("专业级 WEBM 转 MP4 工具，支持批量转换和高质量输出。")}</div>
        </div>
      </div>

      <div className='tool-ui-container' style={{ padding: '24px', flex: 1 }}>
        <div className="tool-main-content video-converter-layout">
          <div className="tool-left-panel">
            <FileUploaderV2
              ...
              showAudioInfo={true}
              uploadPlaceholder={t("将您的 WEBM 文件拖拽到此处")}
            ...
            <SettingsPanel title={t("转换选项")}>
              ...
              <div style={{ marginTop: '20px', paddingTop: '16px', borderTop: '1px solid #f1f5f9' }}>
                <button
                  ...
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg>{t("高级设置")}
        ...
        <div className="confirm-modal" onClick={(e) => e.stopPropagation()} style={{
          ...
          <div className="confirm-header" style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
              ...
              <span style={{ fontSize: '16px', fontWeight: '600', color: 'var(--text-primary)' }}>{t("高级编码参数")}</span>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '32px' }}>
              ...
              <div className="setting-group">
                <div className="setting-header" style={{ marginBottom: '16px' }}>
                  <span className="setting-label" style={{ fontSize: '14px', fontWeight: '600', color: 'var(--text-primary)' }}>{t("视频压缩模式")}</span>
                </div>
                ...
                <div className="compression-mode-content" style={{ minHeight: '80px' }}>
                  ...
                  <div className="setting-sub-item">
                      <div className="setting-sub-label" style={{ fontSize: '13px', marginBottom: '8px', color: 'var(--text-secondary)' }}>{t("目标视频码率")}</div>
                      <select
                      ...
                      onChange={(e) => {
                        ...
                        <option value="1000k">{t("1000k (低质量)")}</option>
                        <option value="2000k">{t("2000k (中等质量)")}</option>
                        <option value="4000k">{t("4000k (高质量)")}</option>
                        <option value="6000k">{t("6000k (超清)")}</option>
                        <option value="8000k">{t("8000k (极清)")}</option>
                        <option value="12000k">{t("12000k (4K级别)")}</option>
                      </select>
                    </div>
                  }
                </div>
              ...
              <div className="setting-group" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div>
                  <div className="setting-header" style={{ marginBottom: '16px' }}>
                    <span className="setting-label" style={{ fontSize: '14px', fontWeight: '600', color: 'var(--text-primary)' }}>{t("关键帧与音频控制")}</span>
                  </div>
                ...
                <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '16px' }}>
                  <div className="audio-setting-content">
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                      <div className="setting-sub-item">
                        <div className="setting-sub-label" style={{ fontSize: '12px', marginBottom: '6px', color: 'var(--text-secondary)' }}>{t("音频采样率")}</div>
                        <select
                          className="custom-select"
                      ...
                      <div className="setting-sub-item">
                        <div className="setting-sub-label" style={{ fontSize: '12px', marginBottom: '6px', color: 'var(--text-secondary)' }}>{t("声道配置")}</div>
                        <select
                          className="custom-select"
                          style={{ width: '100%', height: '36px', borderRadius: '8px', border: '1px solid var(--border-color)', background: 'var(--bg-color)', color: 'var(--text-primary)', fontSize: '13px' }}
                          value={audioChannels}
                          ...
                          <option value="2">{t("立体声 (2.0)")}</option>
                          <option value="1">{t("单声道 (1.0)")}</option>
                        </select>
                      </div>
                    </div>
                  </div>
