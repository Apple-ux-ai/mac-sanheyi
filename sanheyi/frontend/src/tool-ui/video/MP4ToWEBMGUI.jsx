...
const MP4ToWEBMGUI = ({ onBack }) => {
  ...
  const handleFileSelect = async (droppedFiles = null) => {
    let files = [];
    if (droppedFiles) {
      files = Array.from(droppedFiles).map((f) => f.path || f.name);
  ...
  return (
    <div style={{ width: '100%', position: 'relative', height: '100%', overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("MP4 To WEBM 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>{t("\u2014\u2014 专业级 MP4 转 WEBM 工具，支持批量转换和高质量 WebM 格式输出。")}</div>
        </div>
      </div>
      ...
      <div className='tool-ui-container' style={{ padding: '24px', flex: 1 }}>
        <div className="tool-main-content video-converter-layout">
          ...
          <FileUploaderV2
              ...
              showAudioInfo={true}
              uploadPlaceholder={t("将您的MP4文件拖拽到此处")}
              results={results}
      ...
      <ConfirmationModal
        ...
        title={t("取消转换")}
        message={t("确定要取消当前的转换任务吗？已生成的文件可能会被删除。")}
