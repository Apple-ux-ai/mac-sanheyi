...
const AVIToH264GUI = ({ onBack }) => {
  ...
  const handleWebFileChange = (e) => {
    ...
    const aviFiles = files.filter((file) => file.name.toLowerCase().endsWith('.avi'));
    const nonAviCount = files.length - aviFiles.length;

    if (nonAviCount > 0) {
      showAlert('提示', '仅支持选择 AVI 格式的视频文件');
    }

    const newFiles = aviFiles.
    map((f) => f.name).
    filter((name) => !selectedFiles.includes(name));
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("AVI To H264 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px' }}>{t("一款专业工具，可将AVI 视频文件转换为H264(MP4)视频格式，并支持自定义编码参数。")}</div>
        </div>
      </div>
