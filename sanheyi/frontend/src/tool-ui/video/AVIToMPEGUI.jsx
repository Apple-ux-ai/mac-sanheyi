...
const AVIToMPEGUI = ({ onBack }) => {
  ...
  const handleFileSelect = async (droppedFiles = null) => {
    ...
    if (droppedFiles) {
      ...
      paths = allFiles.filter((p) => p.toLowerCase().endsWith('.avi'));
      if (paths.length < allFiles.length) {
    ...
    if (paths && paths.length > 0) {
      ...
      if (!activeFile) setActiveFile(newPaths[0]);
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '8px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("AVI To MPE 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px' }}>{t("一款专业工具，可将AVI 视频文件转换为MPE视频格式，并支持自定义编码参数。")}</div>
        </div>
      </div>
