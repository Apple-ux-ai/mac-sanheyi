...
const WEBMToJPGGUI = ({ onBack }) => {
  ...
  const handleAddFiles = async (newFiles = null) => {
    let filesToProcess = [];
    if (newFiles && (newFiles instanceof FileList || Array.isArray(newFiles))) {
      filesToProcess = Array.from(newFiles).map((f) => f.path || f.name || f);
    ...
    if (validFiles.length < filesToProcess.length && filesToProcess.length > 0) {
      showAlert('提示', '已自动过滤非 WEBM 文件');
    }

    // Add only new files
    const newPaths = validFiles.map((f) => typeof f === 'string' ? f : f.path || f.name);
    const uniqueNewPaths = newPaths.filter((p) => !selectedFiles.includes(p));

    if (uniqueNewPaths.length === 0) return;

    setSelectedFiles((prev) => [...prev, ...uniqueNewPaths]);
      ...
      for (const path of uniqueNewPaths) {
        try {
          ...
          if (result.success) {
            ...
            // Initialize trim settings with full duration
            setTrimSettings((prev) => ({
              ...prev,
              [path]: { startTime: 0, endTime: result.info.duration }
            }));
          }
        } catch (error) {
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("WEBM To JPG 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>{t("从 WEBM 视频中提取高质量 JPG 序列，支持自定义提取间隔和质量。")}</div>
        </div>
      </div>

      <div className='tool-content-scroll'>
        <div className="tool-main-layout video-converter-layout">
