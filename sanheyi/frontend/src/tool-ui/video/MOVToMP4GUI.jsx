...
const MOVToMP4GUI = ({ onBack }) => {
  ...
  const handleFileSelect = async (droppedFiles = null) => {
    ...
    if (droppedFiles) {
      ...
      files = allFiles.filter((f) => f.toLowerCase().endsWith('.mov'));
      if (files.length < allFiles.length) {
        showAlert('提示', '已自动过滤非 MOV 文件');
      }
      ...
      <div style={{ marginBottom: '16px', padding: '0 40px', marginTop: '12px' }}>
        ...
        <div className="header-text" style={{ marginTop: '12px' }}>
           <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '8px' }}>
             ...
             <h1 className="header-title" style={{ fontSize: '24px', fontWeight: '600', margin: 0 }}>{t("MOV To MP4 转换器")}</h1>
           </div>
           <div className="header-desc" style={{ fontSize: '14px', paddingLeft: '44px', color: 'var(--text-secondary)' }}>{t("\u2014\u2014 专业级 MOV 转 MP4 工具，支持批量转换和高质量输出。")}</div>
        </div>
      </div>
        ...
        <div className="tool-main-content video-converter-layout">
          <div className="tool-left-panel">
            <FileUploaderV2
              ...
              showAudioInfo={false}
              uploadPlaceholder={t("将您的 MOV 文件拖拽到此处")}
