...
export function applyConversionNotificationRule(context) {
  ...
  if (rule.allowCustomMessage && payload.customMessage) {
    ...
    const localizedTemplate = typeof rule.defaultMessage === 'string' ? t(rule.defaultMessage) : rule.defaultMessage;
    message = formatTemplate(localizedTemplate, templateData);
  }
