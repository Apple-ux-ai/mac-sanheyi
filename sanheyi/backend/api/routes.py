...
from services.ad_proxy import (
    fetch_ads as fetch_remote_ads,
    DEFAULT_SOFT_NUMBER as AD_DEFAULT_SOFT_NUMBER,
)
...
async def convert_tiff(
    ...
    try:
        ...
        # 保存所有上传文件
        print(f"DEBUG: Received {len(files)} files for TIFF conversion")
        for file in files:
            input_path = await file_handler.save_upload_file(file)
        ...
        # 执行转换（多文件或单文件）
        print(f"DEBUG: Target format: {target_format}, Merge PDF: {merge_pdf}, Input count: {len(input_paths)}")
        if (target_format == 'gif' and len(input_paths) > 1) or \
           (target_format == 'pdf' and len(input_paths) > 1):
            print("DEBUG: Executing MULTI-FILE conversion")
            result = converter_service.convert_tiff(input_paths, target_format, **options)
        else:
            print("DEBUG: Executing SINGLE-FILE conversion")
...
@router.get("/ads")
async def get_ads(
    position: str = Query(..., description="广告位编码，如 adv_position_01"),
    soft_number: str = Query(AD_DEFAULT_SOFT_NUMBER, description="软件标识")
):
    try:
...
@router.get("/ads")
async def proxy_ads(position: str, soft_number: str = AD_DEFAULT_SOFT_NUMBER):
    try:
        ads = await fetch_remote_ads(position, soft_number)
    except Exception as exc:
        raise HTTPException(status_code=502, detail=f"Ad service error: {exc}") from exc
