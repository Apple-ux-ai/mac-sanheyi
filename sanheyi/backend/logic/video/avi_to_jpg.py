...
def generate_preview_video(source_path, output_dir):
    ...
    try:
        ...
        filename = os.path.splitext(os.path.basename(source_path))[0]
        # Use a deterministic name so repeated calls might reuse? 
        # Or unique to avoid conflict? Unique is safer for concurrent.
        # But for preview of same file, reuse is good.
        # Let's use simple hash or just filename_preview.mp4
        output_path = os.path.join(output_dir, f"preview_{filename}.mp4")
